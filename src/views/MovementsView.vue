<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col cols="12">
        <v-list>
          <v-list-item v-for="movement in movements" :key="movement.id">
            <v-card
              density="compact"
              ...
              elevation="5"
            >
              <v-card-title class="text-h5">{{ formatCurrency(movement.value) }} - {{ formatMovementStatus(movement.status) }}</v-card-title>
                ...
                <v-list-item class="w-100 px-2">
                  ...
                  <template v-slot:append>
                    <div class="justify-self-end">
                      <v-btn icon="mdi-trash-can" @click="confirmDeleteMovement(movement.id)"></v-btn>
...
async function fetchMovements() {
  try {
    const { data } = await http.get(`/movements/current-month?category_id=${categoryId.value}&type=${type.value}`)
